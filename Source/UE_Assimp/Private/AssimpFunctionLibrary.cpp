...
#pragma region Windows
    ...
    // Convert the forward slashes in the path name to backslashes, otherwise it'll be ignored as invalid and use
    // whatever is cached in the registry
    ...
    // Convert the "|" delimited list of filetypes to NULL delimited then add a second NULL character to indicate the
    // end of the list
    ...
    // where the second string in the pair is the extension.  To get the clean extensions we only care about the second
    // string in the pair
    ...
    for (int32 ExtensionIndex = 1; ExtensionIndex < UnformattedExtensions.Num(); ExtensionIndex += 2)
    {
        ...
        // Assume the user typed in an extension or doesnt want one when using the *.* extension. We can't determine
        // what extension they wan't in that case
        if (Extension != TEXT("*.*"))
        {
    ...
    if (bSuccess)
    {
        ...
        FString Extension =
            CleanExtensionList.IsValidIndex(OutFilterIndex) ? CleanExtensionList[OutFilterIndex] : TEXT("");
        ...
        for (auto OutFilenameIt = OutFilenames.CreateIterator(); OutFilenameIt; ++OutFilenameIt)
        {
            ...
            {
                // filename does not have an extension. Add an extension based on the filter that the user chose in the
                // dialog
